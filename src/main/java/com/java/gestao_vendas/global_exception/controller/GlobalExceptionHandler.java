package com.java.gestao_vendas.global_exception.controller;

import com.java.gestao_vendas.global_exception.model.ErroPadronizado;
import com.java.gestao_vendas.global_exception.model.ErroValidacao;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErroPadronizado> lidarComCredenciaisInvalidas(BadCredentialsException ex,
                                                                        WebRequest request) {
        ErroPadronizado erro = new ErroPadronizado(
                LocalDateTime.now(),
    ...
    public ResponseEntity<ErroPadronizado> lidarComBloqueioPorCredenciaisInvalidas(LockedException ex,
                                                                        WebRequest request) {
        ErroPadronizado erro = new ErroPadronizado(
                LocalDateTime.now(),
                HttpStatus.UNAUTHORIZED.value(),
                "Usuário bloqueado por múltiplas tentativas inválidas. Tente daqui 10 minutos.",
                request.getDescription(false)
        );
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(erro);
    }
